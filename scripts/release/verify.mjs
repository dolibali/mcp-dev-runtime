...
try {
  ...
  await assert.rejects(cli(['start']), /Missing CONTROL_PLANE/); pass('missing credentials fail locally without a fabricated ready state');
...
process.on('SIGTERM',()=>s.close(()=>process.exit(0)));\n`, { mode: 0o755 });
  ...
  const first = JSON.parse((await cli(['start', '--bg'])).stdout);
  assert.equal(first.state, 'ready'); assert(first.control_socket.length <= 100);
  const restarted = JSON.parse((await cli(['restart', '--bg'])).stdout);
  assert.equal(restarted.state, 'ready'); assert.notEqual(restarted.run_id, first.run_id);
  const legacyUp = JSON.parse((await cli(['up', '--background'])).stdout);
  assert.equal(legacyUp.already_running, true); assert.equal(legacyUp.run_id, restarted.run_id);
  ...
  await cli(['stop']);
  assert.equal(JSON.parse((await cli(['down'])).stdout).state, 'stopped');
