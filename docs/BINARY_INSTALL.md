...
## Optional local Skills in v1.1.0

Skills remain disabled after install/upgrade. See [Local Skills](SKILLS.md) before adding `discover_skills` and `read_skill` to the existing `tools.allow`. Restart the selected installation only after checking active work, then refresh the MCP app tools. No native Skill-upload interface is required; automatic selection by ChatGPT remains a separate hosted-client acceptance check.
