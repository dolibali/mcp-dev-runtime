...
The `MDR_SIGNING_MODE=skip` setting intentionally omits publisher signing and Apple notarization for v1.2.0. `scripts/release/sign.mjs` is the reserved boundary before the manifest/archive hashes are generated; any other mode currently fails closed. A future signer must be reviewed and supplied through protected release credentials. GitHub build provenance is separate from OS signing.
