...
适用版本：1.2.0 / 契约 3.1。六个工具名称和原有正常调用方式不变，追加历史、筛选与日志查询参数；结构化 `output` 和原有文本格式继续保留。严格缓存输出 Schema 的客户端应刷新工具定义。Windows 的原生退出码和 Job 终止不会伪装成 Unix 信号，见 [Windows 说明](WINDOWS.zh-CN.md)。
